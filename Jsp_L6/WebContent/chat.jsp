...
%>
<%
	if(session.getAttribute("logout2") != null){
		session.removeAttribute("logout2");
%>
<script>
window.top.location.href = "login.jsp";
</script>
...
<%
	...
	}else if((String)request.getAttribute("adminAccount") != null){
		account = (String)request.getAttribute("account");
		session.setAttribute("account", account);
		String password = (String)request.getAttribute("password");
		session.setAttribute("password", password);
...
<table id="mainF" width="1014" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
		...
		<!--聊天信息显示窗口 开始-->
		<div id="content">
		     <iframe  width="600px" height="10000px" frameborder="0"  src="chatinfo.jsp"></iframe>
		</div>
		<!--聊天信息显示窗口 结束-->
		</td>
		...
		<div style="margin:10px 0 0 10px;width:195px;line-height:150%;">
		    <iframe src="notice.jsp" scrolling="no"  frameborder="0" height="150px" width="200px"></iframe>
  		</div> 
  		<!--广告模版 结束-->
		</td>
	...
	<tr>
		...
		<!--在线用户列表开始-->
		<div id="user_online">
		    <iframe  width="170px" height="320px" frameborder="0"  src="online.jsp"></iframe>
		</div>
		<!--在线用户列表结束-->
		</td>
	...
	<tr>
		...
		<%
			...
			if(msg != null){
		%>
		<div id="talkcontent"><input type="text" value="<%= msg%>" style="height:20px;width:750px;"/></div>
	...
	<tr>
		...
		<td rowspan="2">
		<!--信息发送按钮 开始-->
		<a id="send" href="#" onClick="sendMessage(event,this);"><img src="img/images/talk_11.gif" width="76" height="38" alt="" /></a>
		<!--信息发送按钮 结束-->
		</td>
